...
mtp: parse.o lex.o mtp.o tree.o hash.o analyze.o codeGen.o
	$(CC) $(LDFLAGS) -o $@ $^ $(LIBS)
lex.o: lex.c lex.h globals.h
	$(CC) $(CFLAGS) -c -o $@ $<
mtp.o: mtp.c lex.h parse.h globals.h analyze.h
	$(CC) $(CLFAGS) -c -o $@ $<
codeGen.o: codeGen.c codeGen.h globals.h hash.h tree.h
	$(CC) $(CFLAGS) -c -o $@ $<
analyze.o: analyze.c analyze.h globals.h tree.h hash.h
	$(CC) $(CFLAGS) -c -o $@ $<
...
clean:
	$(RM) {mtp,lex,parse,tree,hash,analyze,codeGen}.o lex.{c,h} parse.{c,h} mtp
